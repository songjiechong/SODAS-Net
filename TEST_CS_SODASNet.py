...
# parser.add_argument('--epoch_num', type=int, default=600, help='epoch number of model')
...
parser.add_argument('--model_dir', type=str, default='/userhome/SODAS-Net/model', help='trained or pre-trained model directory')
parser.add_argument('--data_dir', type=str, default='/userhome/Dataset', help='training or test data directory')
...
Phi_data_Name = '/userhome/%s/phi_0_%d_1089.mat' % (args.matrix_dir, cs_ratio)
...
# CS_SI_ASSB_noconv_alpha_dataloader_deblock_layer_25_ratio_10_patch_99

model_files = glob.glob(os.path.join(model_dir, '*.pkl'))
epoch_num = len(model_files)
results_csv = []
Phi = torch.from_numpy(Phi_input).type(torch.FloatTensor)
Phi = Phi.to(device)
# Qinit = torch.from_numpy(Qinit_input).type(torch.FloatTensor)
# Qinit = Qinit.to(device)
print('\n')
print("MRI CS Reconstruction Start")
for i in range(len(model_files)):
    print("model:", model_files[i])
    model_path = model_files[i]
    model_index = model_path.split('_')[-1].split('.')[0] 
...
#             cv2.imwrite("%s_SODAS_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, epoch_num, rec_PSNR, rec_SSIM), im_rec_rgb)
            del x_output
...
#     print('\n')
    output_data = "CS ratio is %d, Avg PSNR/SSIM for %s is %.2f/%.4f, Epoch number of model is %s \n" % (cs_ratio, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), str(model_index))
    print(output_data)
    result_csv = [model_index] + [np.mean(PSNR_All)] + [np.mean(SSIM_All)]
    results_csv.append(result_csv)

    
test_result = pd.DataFrame(results_csv)
test_name = "%s/PSNR_SSIM_Results_CS_%s_%s_layer_%d_ratio_%d_epoch_%d.csv" % (model_dir, args.algo_name, test_name, layer_num, cs_ratio, epoch_num*5)
